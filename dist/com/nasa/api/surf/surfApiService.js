...
class SurfApiService {
    ...
    static async createSurf(accessToken, surfData, headers = (0, headers_1.buildHeaders)(), agent) {
        return ApiClient_1.ApiClient.createSignedClient(headers, agent).post(`${SURF_API_BASE_URL}/api/surf/create`, ApiClient_1.ApiClient.buildPayload(surfData), {
            headers: { ...headers, Authorization: `Bearer ${accessToken}` },
        });
    }
    static async completeSurf(accessToken, surfData, headers = (0, headers_1.buildHeaders)(), agent) {
        return ApiClient_1.ApiClient.createSignedClient(headers, agent).post(`${SURF_API_BASE_URL}/api/surf/complete`, ApiClient_1.ApiClient.buildPayload(surfData), {
            headers: { ...headers, Authorization: `Bearer ${accessToken}` },
        });
    }
    static async getSurfHome(accessToken, headers = (0, headers_1.buildHeaders)(), surfId = "", createdAt = Math.floor(Date.now() / 1000), limit = 4, agent) {
        return ApiClient_1.ApiClient.createSignedClient(headers, agent).post(`${SURF_API_BASE_URL}/api/surf/home`, ApiClient_1.ApiClient.buildPayload({ surfId, createdAt, limit }), {
            headers: { ...headers, Authorization: `Bearer ${accessToken}` },
